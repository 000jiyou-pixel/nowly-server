...
# 🛡️ 강력한 브라우저 위장 헤더 (Velog, 알라딘 차단 우회용)
BROWSER_HEADERS = {
...
def get_realtime_keywords():
    ...
    except Exception as e: return DEFAULT_KEYWORDS
...
def get_apple_music_trends():
    try:
        data = requests.get("https://rss.applemarketingtools.com/api/v2/kr/music/most-played/10/songs.json", headers=BROWSER_HEADERS, timeout=10).json()
...
def get_apple_podcast_trends():
    try:
        data = requests.get("https://rss.applemarketingtools.com/api/v2/kr/podcasts/top/10/podcasts.json", headers=BROWSER_HEADERS, timeout=10).json()
        return [{'rank': i+1, 'title': p['name'], 'artist': p['artistName'], 'image': p['artworkUrl100']} for i, p in enumerate(data.get('feed', {}).get('results', []))]
    except Exception as e: return [{"error": str(e)}]

# ==========================================
# 💻 3. IT & 개발 API (Velog 우회 적용, 깃허브)
# ==========================================
def get_velog_trends():
    ...
    try:
        ...
        if not data.get('data', {}).get('trendingPosts'):
            return [{"error": "Velog 서버에서 IP 접근을 차단했습니다."}]
...
def get_hackernews_trends():
    try:
        ...
        for i, sid in enumerate(ids):
            item = requests.get(f"https://hacker-news.firebaseio.com/v0/item/{sid}.json", headers=BROWSER_HEADERS, timeout=5).json()
...
def get_coingecko_trends():
    try:
        data = requests.get("https://api.coingecko.com/api/v3/search/trending", headers=BROWSER_HEADERS, timeout=10).json()
...
# ==========================================
# 🎬 6. 미디어 & 도서 API (영화, TMDB 추가, 알라딘 우회)
# ==========================================
def get_kofic_trends():
    if not KOFIC_API_KEY: return [{"error": "KOFIC_API_KEY 확인 필요"}]
...
def get_tmdb_trends():
    if not TMDB_API_KEY: return [{"error": "TMDB_API_KEY 확인 필요"}]
...
def get_book_rss_trends():
    ...
    try:
        ...
        if not items:
            return [{"error": "알라딘 서버에서 IP 접근을 차단했습니다."}]
            
        return [{'rank': i+1, 'title': item.find('title').text, 'url': item.find('link').text} for i, item in enumerate(items)]
    except Exception as e: return [{"error": str(e)}]
...
def get_trends():
    try:
        ...
        return jsonify({
            ...
            'ott':           get_cached_data('tmdb', get_tmdb_trends), # TMDB 연동 완료
