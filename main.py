...
def get_news_keywords():
    ...
    stopwords = {'이번', '지난', '오늘', '내일', '올해', '지금', '우리', '이후', '이전', 
                 ...
                 '에도', '에게', '부터', '까지', '에는', '에서', '으로', '이다', '있다',
                 '했다', '한다', '된다', '있는', '하는', '하고', '되고', '이고', '뉴스',
                 '기자', '제공', '저작', '무단', '재배', '금지', '서울', '전재', '복제'}
    
    words = [w for w in words if w not in stopwords and len(w) >= 2]
    counter = Counter(words)
    return counter.most_common(20)

# 네이버 데이터랩으로 키워드 검색량 확인
...
완료되면 1~2분 후 아래 주소 접속해주세요!
```
https://nowly-server-production.up.railway.app/trends
